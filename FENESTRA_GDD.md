...
### 7. Círculo deslizante

Variante de pulso onde **o círculo se move lateralmente após o pouso**, em direção às paredes de spikes.
- Usa apenas a mecânica de pulso (sem arcos bloqueados — `blocked_arcs = []`) — o círculo completo pisca ativo/inativo
- Ao pousar, o círculo começa a derivar horizontalmente em direção aleatória (esquerda ou direita) a `drift_speed` px/s
- **Paredes de spikes** aparecem nas bordas da tela — desenhadas num `CanvasLayer` (screen space fixo, não afetadas pela câmera)
- O player se move junto com o círculo enquanto está sobre ele
- Quando a borda do círculo toca a borda da tela → **explosão → player morre**
- A colisão com as paredes é calculada em world space usando a posição real da câmera + zoom: `borda_tela_world = cam_pos.x ± (VIEWPORT_W / 2) / zoom`
- Ao sair (saltar para o próximo) ou morrer: o círculo anima suavemente de volta à posição original (tween 0.5s, TRANS_CUBIC / EASE_OUT)
- As linhas de conexão seguem o movimento do círculo em tempo real (`queue_redraw()` a cada frame)
- Controlado pelos flags `drift_enabled` e `drift_speed` em `Circle.gd`; combina com `pulse_enabled` para pressão dupla
- Signals: `drift_exploded()` emitido por `trigger_drift_explode()` (chamado por `Game.gd`); `Game.gd` chama `player.force_die("drift")`
- As spikes só aparecem após o player pousar no primeiro círculo drift (`_first_walls_index`); somem ao respawnar antes desse ponto

### 8. Círculo inflador

Variante do círculo de arco (Nível 1) onde **o anel externo cresce continuamente após o pouso**, em direção às paredes de spikes.
- Usa arcos bloqueados normais — o player precisa ler o arco livre E sair antes que o círculo infle demais
- Ao pousar, `_grow_radius` começa em `circle_radius` e incrementa a `grow_speed` px/s por frame
- `arc_visual.circle_radius` é atualizado a cada frame para refletir o crescimento visualmente
- Quando a borda do círculo crescido toca a borda da tela → **explosão → player morre**
- A colisão é calculada em world space com compensação de zoom: `circle.position.x ± _grow_radius` vs `cam_pos.x ± VIEWPORT_W/2/zoom`
- Ao sair ou morrer: tween 0.4s (TRANS_CUBIC / EASE_OUT) que anima `arc_visual.circle_radius` de volta ao `circle_radius` original
- A shape de colisão (pouso/saída) **não** cresce — mantém o raio original para gameplay consistente
- Controlado pelos flags `grow_enabled` e `grow_speed` em `Circle.gd`
- Signal `grow_exploded()` emitido por `trigger_grow_explode()` (chamado por `Game.gd`); `Game.gd` chama `player.force_die("grow")`
- As spikes dos níveis 7 e 8 compartilham o mesmo `SpikeWalls` — visíveis **apenas** quando o círculo atual tem `drift_enabled` ou `grow_enabled` (controlado por `Game.gd` em `_on_player_landed`)

### 9. Dupla rotação

Variante com **dois anéis concêntricos girando de forma independente**, cada um com seu próprio padrão de arcos bloqueados.
- O anel interno (raio normal) é o `RotationRoot/ArcVisual` original
- O anel externo (raio + 18px) é criado em `_ready()` via `rotation_root.duplicate()` com `blocked_arcs_b` próprios
- Velocidade e direção de cada anel são independentes — `rotation_speed` (anel A) e `rotation_speed_b` (anel B)
- O player só pode pousar/sair quando as janelas livres de **ambos os anéis** estão simultaneamente alinhadas
- A colisão do anel B é verificada tanto em `is_landing_valid()` (Circle.gd) quanto em `_arc_is_blocked()` (Player.gd)
- Randomização: raio 48–80px; velocidades ±50–95°/s independentes para cada anel; padrões de arco simplificados (1 arco por anel, 90–100° bloqueados)
- Controlado pelos flags `dual_enabled`, `rotation_speed_b`, `blocked_arcs_b` em `Circle.gd`

### 10. Inversão de rotação

Variante onde **a direção de rotação inverte ao pousar e continua invertendo periodicamente com intervalos irregulares**.
- Ao pousar: `rotation_speed = -rotation_speed` imediatamente + flash amarelo ao redor do círculo
- A cada intervalo (1.0–2.2s, **re-sorteado aleatoriamente após cada inversão**): inverte de novo + flash
- A irregularidade elimina ritmo previsível e evita que o player fique "preso" aguardando uma janela que não chega
- O flash amarelo (anel de 0.25s) é o sinal visual único de que a inversão ocorreu
- Sem sinal de explosão — morte ocorre pelo mecanismo normal de arco bloqueado
- Controlado pelos flags `reverse_enabled` e `reverse_interval` em `Circle.gd`
- API: `start_reversing()` (chamado no pouso), `stop_reversing()` (chamado ao sair/morrer)

### 11. Deriva com arco

Combinação das mecânicas de **deriva lateral** (nível 7) com **arcos bloqueados** (nível 1), sem pulso.
- O círculo deriva lateralmente ao pousar (como nível 7), mas em vez do círculo inteiro piscar ativo/inativo, existe um arco bloqueado giratório
- O player precisa ler a janela do arco E sair antes que o círculo encoste nas spikes — dois tempos simultâneos
- Sem pulso: `drift_enabled=true`, `pulse_enabled=false` (default) → `_apply_random_arc()` detecta a combinação pelo branch `drift_enabled and not pulse_enabled`
- As paredes de spikes aparecem normalmente (drift_enabled = true)
- Randomização: raio 48–80px, vel. ±50–95°/s, drift 25–50px/s, 5 padrões de arco do Nível 1
- Controlado pelos flags `drift_enabled`, `drift_speed` e `blocked_arcs` (não vazia) em `Circle.gd`

### 12. Deriva com espelho

Combinação das mecânicas de **deriva lateral** (nível 7) com **espelho** (nível 5).
- A cada pouso: rotação inverte, verde/cinza trocam (lógica mirror) E o círculo começa a derivar lateralmente
- O player precisa reler a janela invertida enquanto o círculo já está se movendo em direção à parede
- Pressão tripla: leitura de arco, reaprendizado de inversão, urgência espacial
- As paredes de spikes aparecem (drift_enabled = true)
- Randomização: raio 48–80px, vel. ±60–110°/s (range mirror), drift 25–50px/s, 5 padrões de arco
- Controlado pelos flags `drift_enabled`, `mirror_mode` e `drift_speed` em `Circle.gd`
- `_apply_random_arc()` detecta a combinação pelo branch `drift_enabled and mirror_mode` (prioritário sobre `drift_enabled and not pulse_enabled`)
...
| 7 | Círculo deslizante (deriva lateral) | ✅ vel. 35–80°/s, drift 25–50px/s, 5 padrões de timing | Urgência espacial + pulso simultâneos |
| 8 | Círculo inflador (crescimento lateral) | ✅ raio 48–80px, vel. 50–95°/s, grow 8–18px/s, 5 padrões de arco | Urgência espacial + leitura de arco simultâneos |
| 9 | Dupla rotação (dois anéis independentes) | ✅ raio 48–80px, vel. ±50–95°/s por anel, padrões simplificados | Sincronização de duas janelas simultâneas |
...
│   │   ├── CircleDrift.tscn           # perigo: deriva lateral em direção às paredes de spikes
│   │   ├── CircleGrow.tscn            # perigo: anel inflador em direção às paredes de spikes
│   │   ├── CircleDual.tscn            # perigo: dois anéis concêntricos com rotações independentes
│   │   ├── CircleReverse.tscn         # perigo: rotação inverte ao pousar e periodicamente
│   │   ├── CircleDriftArc.tscn        # perigo: deriva lateral + arco bloqueado (sem pulso)
│   │   └── CircleDriftMirror.tscn     # perigo: deriva lateral + inversão de arcos a cada pouso
│   ├── entities/
...
| `CircleDrift.tscn` | Deriva lateral em direção às paredes de spikes | `blocked_arcs=[]`, `pulse_enabled=true`, `drift_enabled=true`, `drift_speed=40` |
| `CircleGrow.tscn` | Anel inflador em direção às paredes de spikes | `circle_radius=65`, `blocked_arcs=[30°,150°]`, `grow_enabled=true`, `grow_speed=12` |
| `CircleDual.tscn` | Dois anéis concêntricos independentes | `circle_radius=65`, `dual_enabled=true`, `rotation_speed_b=-85`, `blocked_arcs_b=[200°,320°]` |
| `CircleReverse.tscn` | Rotação inverte ao pousar e periodicamente | `circle_radius=68`, `reverse_enabled=true`, `reverse_interval=1.5` |
| `CircleDriftArc.tscn` | Deriva lateral + arco bloqueado (sem pulso) | `circle_radius=68`, `drift_enabled=true`, `drift_speed=40`, `blocked_arcs=[30°,150°]` |
| `CircleDriftMirror.tscn` | Deriva lateral + inversão de arcos a cada pouso | `circle_radius=65`, `drift_enabled=true`, `drift_speed=35`, `mirror_mode=true`, `blocked_arcs=[30°,150°]` |
...
@export var drift_enabled: bool            # true = círculo deriva lateralmente ao pousar
@export var drift_speed: float             # px/s de movimento horizontal (randomizado: 25–50)
@export var grow_enabled: bool             # true = anel cresce ao pousar em direção às spikes
@export var grow_speed: float              # px/s de crescimento do raio (randomizado: 8–18)
@export var dual_enabled: bool             # true = dois anéis concêntricos girando independentemente
@export var rotation_speed_b: float        # graus/s do anel externo (independente de rotation_speed)
@export var blocked_arcs_b: Array[Vector2] # arcos bloqueados do anel externo
@export var reverse_enabled: bool          # true = rotação inverte ao pousar e periodicamente
@export var reverse_interval: float        # segundos entre inversões (re-sorteado a cada inversão: 1.0–2.2s)
...
- `start_drifting()` — inicia a deriva; direção (±) sorteada aleatoriamente
- `stop_drifting()` — para a deriva e anima o círculo de volta à posição original (tween 0.5s)
- `trigger_drift_explode()` — chamado por `Game.gd` quando borda do círculo toca borda da tela; emite `drift_exploded`
- `start_growing()` — inicia crescimento: `_grow_radius = circle_radius`, incrementa a cada frame
- `stop_growing()` — para o crescimento e anima `arc_visual.circle_radius` de volta ao original (tween 0.4s)
- `trigger_grow_explode()` — chamado por `Game.gd` quando borda inflada toca borda da tela; emite `grow_exploded`
- `start_reversing()` — inverte `rotation_speed`, sorteia `reverse_interval`, inicia inversões periódicas + flash
...
## Demo — 12 níveis (implementado em Game.tscn)

49 círculos ao todo. Cada nível introduz exatamente um tipo de perigo novo (ou uma combinação nova de mecânicas existentes). Os checkpoints (bg_number visível) são ponto de respawn e separação visual de fase.

Níveis 1, 2, 4, 5, 6, 7, 8, 9, 10, 11 e 12 são **randomizados a cada run** via `level_randomize = true`. Os valores nas tabelas abaixo são os ranges de randomização, não valores fixos.
...
### Nível 7 — Deslizante *(randomizado)*

| Círculo | Posição | Raio | Vel. (°/s) | Ativo / Inativo | Drift (px/s) |
|---------|---------|------|------------|-----------------|--------------|
| DriftA | (175, −10350) | 62 | ±35–80 | 1 de 5 padrões | 25–50 |
| DriftB | (215, −10800) | 62 | ±35–80 | 1 de 5 padrões | 25–50 |
| DriftC | (190, −11250) | 62 | ±35–80 | 1 de 5 padrões | 25–50 |
| CP7 | (195, −11700) | 72 | 0 | — | — |

**5 padrões de timing disponíveis** (mesmos do Nível 2):
- p0: 1.8s / 0.7s — generoso
- p1: 1.5s / 0.9s — equilibrado
- p2: 0.7s / 1.4s — janela ativa curta
- p3: 2.0s / 0.5s — ativo longo, inativo rápido
- p4: 1.0s / 1.2s — inativo dominante

> Sem arcos bloqueados — o círculo inteiro é livre ou inativo. A pressão é dupla: o pulso define a janela para sair, e a deriva lateral empurra o círculo (e o player) em direção à parede de spikes. As spikes aparecem fixas nas bordas da tela ao entrar no nível. A direção da deriva é aleatória a cada pouso. Ao sair ou morrer, o círculo anima suavemente de volta à posição original.

### Nível 8 — Inflador *(randomizado)*

| Círculo | Posição | Raio | Vel. (°/s) | Grow (px/s) |
|---------|---------|------|------------|-------------|
| GrowA | (155, −12150) | 48–80 | ±50–95 | 8–18 |
| GrowB | (235, −12600) | 48–80 | ±50–95 | 8–18 |
| GrowC | (160, −13050) | 48–80 | ±50–95 | 8–18 |
| CP8 | (195, −13500) | 72 | 0 | — |

**5 padrões de arco disponíveis** (mesmos do Nível 1):
- p0: `[30°, 150°]` — 120° bloqueado, janela grande
- p1: `[15°, 175°]` — 160° bloqueado, janela média
- p2: `[200°, 340°]` — 140° bloqueado, posição diferente
- p3: `[10°, 70°]` + `[190°, 250°]` — 2 arcos simétricos
- p4: `[40°, 110°]` + `[210°, 285°]` — 2 arcos offset

> Os círculos são posicionados mais ao centro (x=155–235) para dar margem simétrica às paredes. As spikes continuam visíveis desde o Nível 7. O desafio combina a leitura de arco do Nível 1 com urgência espacial crescente — o player precisa agir antes de o círculo inflar e encostar nas bordas. Ao sair ou morrer, o raio anima suavemente de volta ao tamanho original.

### Nível 9 — Dupla rotação *(randomizado)*

| Círculo | Posição | Raio | Vel. A (°/s) | Vel. B (°/s) | Padrão |
|---------|---------|------|-------------|-------------|--------|
| DualA | (108, −13950) | 48–80 | ±50–95 | ±50–95 | 1 de 5 padrões simplificados por anel |
| DualB | (282, −14400) | 48–80 | ±50–95 | ±50–95 | 1 de 5 padrões simplificados por anel |
| DualC | (115, −14850) | 48–80 | ±50–95 | ±50–95 | 1 de 5 padrões simplificados por anel |
| CP9 | (195, −15300) | 72 | 0 | — | Checkpoint (bg_number=10) |

**5 padrões simplificados (por anel, 1 arco por anel, 90–100° bloqueados):**
- p0: `[40°, 130°]` — 90° bloqueado
- p1: `[20°, 120°]` — 100° bloqueado
- p2: `[210°, 300°]` — 90° bloqueado
- p3: `[150°, 250°]` — 100° bloqueado
- p4: `[80°, 175°]` — 95° bloqueado

> Dois anéis concêntricos giram em velocidades e direções independentes. O anel externo é desenhado 18px além do raio base. A janela de saída só existe quando os dois estão simultaneamente alinhados — essa sincronização varia a cada momento e é o desafio central. Padrões simplificados (1 arco por anel, ~90–100° bloqueados) garantem janelas amplas mesmo com dois anéis ativos.

### Nível 10 — Inversão *(randomizado)*

| Círculo | Posição | Raio | Vel. (°/s) | Intervalo de inversão |
|---------|---------|------|------------|----------------------|
| RevA | (108, −15750) | 48–80 | ±50–95 | 1.0–2.2s (re-sorteado) |
| RevB | (282, −16200) | 48–80 | ±50–95 | 1.0–2.2s (re-sorteado) |
| RevC | (115, −16650) | 48–80 | ±50–95 | 1.0–2.2s (re-sorteado) |
| CP10 | (195, −17100) | 72 | 0 | — |

> Ao pousar, a rotação inverte imediatamente (flash amarelo de 0.25s indica a inversão). Continua invertendo periodicamente com intervalos **irregulares** — cada intervalo é re-sorteado entre 1.0 e 2.2s após cada inversão. A irregularidade elimina ritmo previsível e garante que janelas longas ocasionais existam para o player sair. Usa os mesmos 5 padrões de arco do Nível 1.

### Nível 11 — Deriva com arco *(randomizado)*

| Círculo | Posição | Raio | Vel. (°/s) | Drift (px/s) | Padrão de arco |
|---------|---------|------|------------|--------------|----------------|
| DriftArcA | (108, −17550) | 48–80 | ±50–95 | 25–50 | 1 de 5 padrões |
| DriftArcB | (282, −18000) | 48–80 | ±50–95 | 25–50 | 1 de 5 padrões |
| DriftArcC | (115, −18450) | 48–80 | ±50–95 | 25–50 | 1 de 5 padrões |
| CP11 | (195, −18900) | 72 | 0 | — | Checkpoint (bg_number=12) |
...
> Combina a deriva lateral do Nível 7 com os arcos giratórios do Nível 1, mas sem pulso. Ao pousar, o círculo começa a se mover lateralmente imediatamente enquanto o arco continua girando. O player precisa ler a janela do arco E sair antes que o círculo encoste nas spikes — dois deadlines simultâneos de naturezas diferentes. As spikes aparecem normalmente.

### Nível 12 — Deriva com espelho *(randomizado)*

| Círculo | Posição | Raio | Vel. (°/s) | Drift (px/s) | Mirror |
|---------|---------|------|------------|--------------|--------|
| DriftMirA | (175, −19350) | 48–80 | ±60–110 | 25–50 | 1 de 5 padrões de arco |
| DriftMirB | (215, −19800) | 48–80 | ±60–110 | 25–50 | 1 de 5 padrões de arco |
| DriftMirC | (190, −20250) | 48–80 | ±60–110 | 25–50 | 1 de 5 padrões de arco |
| CP12 | (195, −20700) | 72 | 0 | — | Checkpoint (bg_number=13) |

**5 padrões de arco disponíveis** (mesmos do Nível 1, velocidade no range mirror 60–110°/s).
...
Itens coletáveis aparecem entre os círculos da sequência, posicionados no ponto médio da linha de conexão.
...
- Garante que itens entre círculos drift seguem o movimento lateral junto com o restante da cena

### Coleta
- Detecção via distância do segmento de movimento do player ao item (não ponto-a-ponto), para não falhar quando o player se move rápido
- `ITEM_COLLECT_RADIUS = 20px`
- Ao coletar: item escala para 1.6× e faz fade out em 0.15s, depois `queue_free()`
- Emite `collected(type: int)` → `Game.gd._on_item_collected(type)`

### Tipos
| Tipo | Constante | Cor | Efeito |
|------|-----------|-----|--------|
...
19. ✅ Círculo deslizante (`CircleDrift.tscn`) — nível 7 implementado (deriva + pulso + paredes de spikes em CanvasLayer)
20. ✅ Círculo inflador (`CircleGrow.tscn`) — nível 8 implementado (arcos + crescimento lateral até as spikes; spikes compartilhadas com nível 7)
21. ✅ Círculo de dupla rotação (`CircleDual.tscn`) — nível 9 implementado (dois anéis concêntricos independentes; spikes só aparecem em círculos drift/grow)
22. ✅ Círculo de inversão (`CircleReverse.tscn`) — nível 10 implementado (reversão imediata no pouso + inversões periódicas irregulares com flash amarelo)
