...
- As spikes só aparecem após o player pousar no primeiro círculo drift (`_first_drift_index`); somem ao respawnar antes desse ponto
...
- `_on_player_landed`: atualiza `current_index`; se `mirror_mode` → `flip_mirror()`; se `orbiter_chaser` → `activate_chasers()`; senão `clear_orbiters()`; se `shrink_enabled` → `start_shrinking()` + conecta `shrink_exploded`; se `drift_enabled` → `start_drifting()` + conecta `drift_exploded`; mostra spikes se `current_index >= _first_drift_index`; salva checkpoint se `bg_number > 0`
- `_on_player_died`: libera chasers/shrink/drift se necessário; decrementa `lives`; se 0 → `show_game_over()`; senão respawn no último checkpoint após 1s; oculta spikes se checkpoint anterior ao nível drift; chama `_reset_circles_after_checkpoint()`
- `_on_shrink_exploded` / `_on_drift_exploded`: chamam `player.force_die(reason)`
- `_follow_drift_circle()`: a cada frame, sincroniza `player.global_position` com o círculo drift em movimento; verifica colisão com bordas da tela (world space = `cam_pos.x ± VIEWPORT_W/2/zoom`); chama `trigger_drift_explode()` se necessário; aciona `queue_redraw()` durante deriva e animação de retorno
- `SpikeLayer/SpikeWalls` (`SpikeWalls.gd`): `Node2D` dentro de `CanvasLayer` (layer=1); desenha paredes de spikes em screen space (x=0 e x=390), fixas e independentes da câmera; visibilidade controlada por `Game.gd`
...
## Demo — 7 níveis (implementado em Game.tscn)

29 círculos ao todo. Cada nível introduz exatamente um tipo de perigo novo. Os checkpoints (bg_number visível) são ponto de respawn e separação visual de fase.

Níveis 1, 2, 4, 5, 6 e 7 são **randomizados a cada run** via `level_randomize = true`. Os valores nas tabelas abaixo são os ranges de randomização, não valores fixos.
...
20. Feedback visual/sonoro de morte diferenciado (blocked = vermelho seco, inactive = fade)
21. `PhaseConfig.gd` como `Resource` para configurar níveis via editor
22. HUD definitivo com score e combo (substituir UI provisória)
23. Testar em dispositivo real desde cedo
