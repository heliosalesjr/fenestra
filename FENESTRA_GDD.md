...
- `_on_player_landed`: atualiza `current_index`; se `orbiter_chaser` → `activate_chasers()`; senão `clear_orbiters()`; salva checkpoint se `bg_number > 0`
...
## Demo — 4 níveis (implementado em Game.tscn)

17 círculos ao todo. Cada nível introduz exatamente um tipo de perigo novo. Os checkpoints (bg_number visível) são ponto de respawn e separação visual de fase.
...
|-------|-----------|--------------|
| **Círculo espelho** | Inverte a direção de rotação toda vez que o player pousa. O que era verde vira bloqueado e vice-versa. Força reaprendizado do timing a cada visita. | Média |
...
14. Indicador visual de progresso do pulso (anel de timer ao redor da borda)
15. Feedback visual/sonoro de morte diferenciado (blocked = vermelho seco, inactive = fade)
16. `PhaseConfig.gd` como `Resource` para configurar níveis via editor
17. HUD definitivo com score e combo (substituir UI provisória)
18. Testar em dispositivo real desde cedo
