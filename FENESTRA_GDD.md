...
- As spikes dos níveis 7 e 8 compartilham o mesmo `SpikeWalls` — `_first_walls_index` cobre o primeiro círculo com `drift_enabled` ou `grow_enabled`
...
- `_on_player_landed`: atualiza `current_index`; handles mirror/chasers/shrink/drift/grow; mostra spikes se `current_index >= _first_walls_index` (primeiro círculo com drift ou grow); salva checkpoint se `bg_number > 0`
- `_on_player_died`: libera chasers/shrink/drift/grow; decrementa `lives`; se 0 → `show_game_over()`; senão respawn no checkpoint; oculta spikes se checkpoint anterior ao primeiro nível com paredes; chama `_reset_circles_after_checkpoint()`
...
- `_first_walls_index`: índice do primeiro círculo com `drift_enabled` ou `grow_enabled`; controla visibilidade das spikes
- `SpikeLayer/SpikeWalls` (`SpikeWalls.gd`): `Node2D` dentro de `CanvasLayer` (layer=1); desenha paredes de spikes em screen space (x=0 e x=390), fixas e independentes da câmera; compartilhado entre níveis 7 e 8; visibilidade controlada por `Game.gd`
...
## Demo — 8 níveis (implementado em Game.tscn)

33 círculos ao todo. Cada nível introduz exatamente um tipo de perigo novo. Os checkpoints (bg_number visível) são ponto de respawn e separação visual de fase.

Níveis 1, 2, 4, 5, 6, 7 e 8 são **randomizados a cada run** via `level_randomize = true`. Os valores nas tabelas abaixo são os ranges de randomização, não valores fixos.
...
21. Feedback visual/sonoro de morte diferenciado (blocked = vermelho seco, inactive = fade)
