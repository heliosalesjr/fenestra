...
> Introdução pura ao timing. CircleStart não tem perigo — serve só para o player entender que o toque lança o movimento. ArcA tem uma janela bloqueada grande (~130°) mas velocidade moderada para dar tempo de ler. ArcB é o primeiro a ter dois arcos, criando uma janela verde estreita entre eles. ArcC tem o maior arco bloqueado do nível (~180°), forçando o player a esperar a janela certa antes de concluir.
...
> Sem arcos bloqueados — o único perigo é o timing do pulso. PulseA é generoso (1.8s ativo / 0.7s inativo): o player aprende a observar e esperar. PulseB inverte a proporção (0.6s ativo / 1.4s inativo), forçando uma janela muito curta e rápida. PulseC tem a janela ativa mais longa do nível mas o inativo mais curto (0.45s), criando uma falsa sensação de segurança — quem espera demais dentro do círculo já morre.
...
> A tensão vem do tempo, não do espaço. Ao pousar, os orbiters ficam vermelhos e se movem em direção ao centro — o player precisa sair antes que algum chegue. ChaserA tem só 2 perseguidores: dá para respirar e aprender o ritmo. ChaserB sobe para 3, a pressão aumenta. ChaserC tem 4 chasers com raio de órbita mais apertado (mult=1.3), então já estão mais perto ao começar a perseguir — a janela de saída fica muito menor.
