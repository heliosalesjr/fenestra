...
const WALL_INSET    := 14.0   # distância das bordas para a eletricidade não ser cortada
...
func _draw() -> void:
	_draw_elec_wall(WALL_INSET)
	_draw_elec_wall(VIEWPORT_W - WALL_INSET)


func _draw_elec_wall(x: float) -> void:
	var n: int = max(8, int(VIEWPORT_H / 6.0))
	var pts := PackedVector2Array()
	for i: int in range(n + 1):
		var t: float = float(i) / float(n)
		var y: float = t * VIEWPORT_H
		var dx: float = randf_range(-7.0, 7.0) if (i > 0 and i < n) else 0.0
